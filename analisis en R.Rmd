...
cor(Money_earned_spent$Av_money_spent, Money_earned_spent$Money_earned_in_thousands, method = 'pearson')
...
#Standard deviation is only 3.17 million transactions 
#Notice the standard error has diminished from 8.2 million transactions to 3.17.
...
working_place_pctg <- c((length(which(survey$remunerado == "Sí" & survey$`ciudad o poblado` == "Ciudad")))/(length(which(survey$remunerado == "Sí")))*100,
(length(which(survey$remunerado == "Sí" & survey$`ciudad o poblado` == "Poblado")))/(length(which(survey$remunerado == "Sí")))*100
                         )

hire_place_pctg <- c((length(which(survey$`dispuesto a contratar` == "Sí" & survey$`ciudad o poblado` == "Ciudad")))/(length(which(survey$`dispuesto a contratar` == "Sí")))*100,
(length(which(survey$`dispuesto a contratar` == "Sí" & survey$`ciudad o poblado` == "Poblado")))/(length(which(survey$`dispuesto a contratar` == "Sí")))*100
                         )

place_of_living <- c("Ciudad", "Poblado")

ppl_places <- data.frame(place_of_living, hire_place_pctg, working_place_pctg)

write_csv2(ppl_places, "D:/proyecto de start up/R proyecto/R_export/people_provenanceDOM.csv")
...
shopping_made_online <- c(Percentage_of_shopping_done_online$Percentage_of_shopping_made_online)
shopping_made_online <- shopping_made_online[-c(1:4)]

servs_users_money_earned <- data.frame(years, GNI, Money_online_shopping, servers, users, Av_spending, shopping_made_online)
...
shapiro.test(servs_users_money_earned$shopping_made_online)$p.value > 0.05    
    ){
...
shapiro.test(servs_users_money_earned$shopping_made_online)$p.value < 0.05           
  ...
  }& if (shapiro.test(servs_users_money_earned$shopping_made_online)$p.value < 0.05){
    print("shopping made online is not normaly distributed")
  }
}
...
#linearity is given
cor(servs_users_money_earned$Money_online_shopping, servs_users_money_earned$shopping_made_online, method = 'pearson')
#linearity is given

multivar_money_earned_model <- lm(Money_online_shopping ~ servers + Av_spending + GNI + shopping_made_online, servs_users_money_earned)
